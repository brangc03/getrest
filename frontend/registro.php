...
// fn util para identificador
function crearSlug($texto, $con) {
    // lim de caracteres
    $limpio = preg_replace('~[^\pL\d]+~u', '-', $texto);
    $limpio = iconv('utf-8', 'us-ascii//TRANSLIT', $limpio);
    $limpio = preg_replace('~[^-\w]+~', '', $limpio);
    $limpio = trim($limpio, '-');
    ...
    // iteracion eval
    while(true) {
        $st = $con->prepare("SELECT Id_Restaurante FROM Restaurante WHERE Slug = ?");
        $st->bind_param("s", $slug_final);
        $st->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt_check->get_result()->num_rows > 0) {
        ...
        // gen id unico
        $slug_nuevo = crearSlug($restaurante, $conexion);

        // iny rest
        $stmt_rest = $conexion->prepare("INSERT INTO Restaurante (Nombre, Slug) VALUES (?, ?)");
        $stmt_rest->bind_param("ss", $restaurante, $slug_nuevo);
        
        if ($stmt_rest->execute()) {
            $id_restaurante = $conexion->insert_id;
            
            // iny usr
            $rol = "gerente";
            $stmt_user = $conexion->prepare("INSERT INTO Usuario (Id_Restaurante, Correo, Contrasena, Rol) VALUES (?, ?, ?, ?)");
            $stmt_user->bind_param("isss", $id_restaurante, $correo, $password, $rol);
            
            if ($stmt_user->execute()) {
                $exito = "Cuenta creada. Tu identificador es <strong>$slug_nuevo</strong>. Ya puedes iniciar sesión.";
            } else {
...
<head>
    ...
    <title>Getrest - Crear Cuenta</title>
    ...
    <div class="h-screen w-full flex" style="font-family: 'DM Sans', sans-serif;">
        <div class="hidden lg:flex flex-col justify-center w-[52%] p-14 relative overflow-hidden" style="background: #990616;">
            ...
            <div class="relative z-10 mb-32 animate-fade-in">
                <p class="text-xs tracking-widest uppercase mb-8" style="color: #F1E9C655;">Únete a nosotros</p>
                <h1 class="leading-[1.15] mb-6" style="font-family: 'Playfair Display', serif; font-size: clamp(2.5rem, 4vw, 3.5rem); font-weight: 400; color: #F1E9C6;">
                    Digitaliza tu<br><em style="color: #A18E5E;">restaurante</em> hoy
                </h1>
            </div>
        </div>
        ...
        <div class="flex-1 flex items-center justify-center p-8" style="background-color: #F4EED1;">
            <div class="w-full max-w-sm">
                <div class="animate-fade-in-up">
                    ...
                    <p class="text-xs tracking-widest uppercase mb-3" style="color: #47474766;">Registrarse</p>
                    <h2 class="mb-8" style="font-family: 'Playfair Display', serif; font-size: 1.75rem; font-weight: 400; color: #474747;">
                        Crea tu cuenta.
                    </h2>
                    ...
                    <?php if ($exito): ?>
                        <div class="mb-6 p-3 rounded-md bg-green-100 text-green-700 text-sm font-medium border border-green-200">
                            <?= $exito ?>
                        </div>
                    <?php endif; ?>
                </div>

                <form method="POST" class="flex flex-col gap-4 animate-fade-in-up delay-100">
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs tracking-wide uppercase font-medium" style="color: #47474788;">Nombre del Restaurante</label>
                        <div class="relative rounded-lg transition-all duration-200 border-[1.5px] border-[#47474722] bg-[#F1E9C6cc] focus-within:border-[#A18E5E] focus-within:bg-[#fffdf4]">
                            <input type="text" name="restaurante" placeholder="Mi Restaurante" required class="w-full bg-transparent px-4 py-3.5 text-sm outline-none" style="color: #474747; font-family: 'DM Sans', sans-serif;">
                        </div>
                    </div>
                    ...
                    <button type="submit" class="relative mt-4 w-full py-3.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all duration-200 overflow-hidden group" style="background: #990616; color: #F1E9C6; font-family: 'DM Sans', sans-serif;">
                ...
                <p class="text-center text-xs mt-8 animate-fade-in-up delay-200" style="color: #47474755;">
                    ¿Ya tienes cuenta? <a href="/getrest/login.php" class="underline underline-offset-2 transition-colors" style="color: #990616; font-family: 'DM Sans', sans-serif;"><span style="font-size: 0.7rem;">Iniciar sesión</span></a>
                </p>
            </div>
        </div>
    </div>
